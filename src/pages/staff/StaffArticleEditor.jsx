...
import { ARTICLE_SELECT, bodyParagraphs, createSlug } from '../../lib/articles.js';
import { getThumbnailUrl, readableError, supabase } from '../../lib/supabase.js';

const emptyArticle = { title: '', excerpt: '', body: '', slug: '', status: 'draft', thumbnail_path: null };
...
export default function StaffArticleEditor() {
  ...
  useEffect(() => {
    if (!id) return;
    let active = true;
    supabase.from('articles').select(ARTICLE_SELECT).eq('id', id).maybeSingle().then(async ({ data, error }) => {
      if (!active) return;
      if (error) setState((value) => ({ ...value, loading: false, error: readableError(error) }));
      else if (!data) setState((value) => ({ ...value, loading: false, missing: true }));
      else {
        setArticle(data);
  ...
  async function save(targetStatus) {
    ...
    if (result.error) {
      if (uploadedPath) await supabase.storage.from('article-thumbnails').remove([uploadedPath]);
      setState((value) => ({ ...value, pending: false, error: readableError(result.error) }));
      return;
    }
    if (initialThumbnail && initialThumbnail !== thumbnailPath) {
      await supabase.storage.from('article-thumbnails').remove([initialThumbnail]);
    }
    ...
    setState((value) => ({ ...value, pending: false, success: targetStatus === 'published' ? 'Article published.' : 'Draft saved.' }));
      ...
      <div className="px-4 py-12 sm:px-6 lg:px-10">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_0.8fr]">
            <form onSubmit={(event) => { event.preventDefault(); save('draft'); }} className="rounded-3xl border border-navy-900/15 bg-white/45 p-6 shadow-soft sm:p-8">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button type="submit" disabled={state.pending} className="secondary-button disabled:opacity-50">{state.pending ? 'Saving…' : 'Save Draft'}</button>
                <button type="button" disabled={state.pending} onClick={() => save('published')} className="primary-button disabled:opacity-50">{state.pending ? 'Working…' : 'Publish Article'}</button>
                {article.status === 'published' ? <button type="button" disabled={state.pending} onClick={unpublish} className="secondary-button disabled:opacity-50">Unpublish</button> : null}
              </div>
            </form>
