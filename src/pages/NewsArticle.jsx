...
import { ARTICLE_SELECT, bodyParagraphs, formatDate } from '../lib/articles.js';
...
export default function NewsArticle() {
  ...
  useEffect(() => {
    ...
    supabase.from('articles').select(ARTICLE_SELECT).eq('slug', slug).eq('status', 'published').maybeSingle()
      .then(async ({ data, error }) => {
        const article = data ? { ...data, thumbnail_url: await getThumbnailUrl(data.thumbnail_path) } : null;
        if (active) setState({ loading: false, article, error: error ? readableError(error) : '', missing: !error && !data });
      });
    return () => { active = false; };
  }, [slug]);
    ...
    <>
      ...
      <article className="px-4 py-12 sm:px-6 sm:py-16 lg:px-10">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-8 text-xs font-semibold uppercase tracking-[0.18em] text-maroon-900">{formatDate(article.published_at)}</p>
